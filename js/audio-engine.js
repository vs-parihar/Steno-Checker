...
au.ontimeupdate=()=>{
    ...
    if(c/d>0.8&&curO.url&&!played80){
        played80=1;
        let id=curO.url.split('details/')[1];
        if(!hist[id])hist[id]=0;
        hist[id]++;
...
async function expA(){
    ...
    if(r===1.0){
        const a=document.createElement('a');a.href=au.src;a.download='Audio.mp3';a.click();
